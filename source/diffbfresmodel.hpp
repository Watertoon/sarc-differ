...
bool DiffBfresModel(dd::res::ResBfresModel *l_model, dd::res::ResBfresModel *r_model, u32 indent_level, bool is_print) {
    ...
    for (u32 i = 0; i < l_shape_iterator.GetFileCount(); ++i) {
        ...
        if (r_index != RomfsDirectoryParser::InvalidIndex) {
            //const bool diff = DiffBfresShape();
            //if (diff == true) { continue; }
            //DiffBfresShape();
        }
    }
